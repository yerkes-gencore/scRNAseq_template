...
## Samples sizes per ``r grp_fct`` level

<!-- Note: Template assumes only one factor (in this case a combination of two factors). For repeated measures design, do `group_by(sample, {{ grp_fct }})`. -->

```{r, message=FALSE, warning=FALSE}
pb[[1]]$dge$samples %>%
  select("sample", all_of(grp_fct)) %>%
  as_tibble() %>%
  group_by(.data[[grp_fct]]) %>%
...
### Scatter plot: split by ``r grp_fct``, colored by ``r clust_labs``, y = ``r clust_labs``
...
### Stacked bar plots; arranged by ``r sampleID`` and ``r grp_fct``
...
plotFilterByExpr(pb_prefilt, keep_tbl, pseudobulk = T) + 
...
```{r ggplotMDS, results='asis'}
chunk_ggplotMDS <- c(
  '## {{ cluster_i }}\n',
  '```{r, fig.width=8, fig.height=6, message=F, warning=F}',
  'ggplotMDS(pb[["{{cluster_i}}"]]$dge, gene.selection = "common",
...
out <- NULL
for (cluster_i in names(pb)) {
  out <- c(out, knitr::knit_expand(text = chunk_ggplotMDS))
}
cat(knitr::knit_child(text = out, quiet = TRUE), sep = '\n')
```
