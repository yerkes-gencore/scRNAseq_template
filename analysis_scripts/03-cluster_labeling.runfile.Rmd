...
```{r}
heatmapClusterADT <- function(x, title, silent = T) {
  ...
  pheatmap::pheatmap(t(adt_plot),
                     ...
                     border_color = NA, silent = silent,
                     legend = FALSE, treeheight_row = 10, treeheight_col = 10,
                     fontsize_row = 12, fontsize_col = 12, main = title) %>% 
    ggplotify::as.ggplot()  +
...
```{r, fig.height=8}
pbmc.heatmaps.adt <- heatmapClusterADT(processed_obj, title = 'ADT Data by cluster')
pbmc.heatmaps.adt
```
...
```{r}
rmarkdown::render(here("analysis_scripts/03-cluster_labeling.format.Rmd"),
                  output_file = '03-cluster_labeling.html',
