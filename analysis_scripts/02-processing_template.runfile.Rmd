...
saveRDS(objs, here('saved_rds/report-02/raw_captures.Rds'))
...
```{r, include=FALSE}
## If human, cc genes were already loaded with Seurat
cc.genes
...
```{r, include=FALSE}
s.split <- lapply(s.split, scoreCC, s.genes = cc_genes$s.genes, g2m.genes = cc_genes$g2m.genes)
```
...
```{r}
saveRDS(objs_filt_mapped, here('saved_rds/report-02/objs_filt_mapped.Rds'))
objs_filt_mapped <- readRDS(here('saved_rds/report-02/objs_filt_mapped.Rds'))
```
...
saveRDS(obj_post_harmony_int,here('saved_rds/report-02/obj_post_harmony.Rds'), compress=FALSE)
obj_post_harmony_int <- readRDS(here('saved_rds/report-02/obj_post_harmony.Rds'))
# rm(pbmc.merged)
gc()
```
