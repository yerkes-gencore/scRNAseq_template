...
```{r}
## This should be in the package now
...
objs <- lapply(objs, scoreCC, s.genes = cc_genes$s.genes, g2m.genes = cc_genes$g2m.genes)
```
...
lapply(objs, ElbowPlot, ndims = 30)
## If nothing egregious with a capture, just use all dims? 
ndims <- 1:30
```
...
umaps_before_doublet_removal <- ggarrange(plotlist = umaps.rna[["doublets"]], ncol=6, nrow=6, 
...
```{r}
objs_filt <- lapply(objs_filt, regress_cc)
```

```{r, message=FALSE, warning=FALSE, fig.width=12, fig.height=12}
umaps.rna[["Phase_post_regression"]] <- objs %>%
  plotUMAP_split(., group.by = "Phase", label.size=0, legend = T, reduction = 'GEX_UMAP')
...
If you removed doublets or regressed cell cycle, you should rerun these
...
```{r}
umaps.rna <- list()
umaps.rna[["clusters"]] <- plotUMAP_split(objs,
...
umaps.rna[["Phase"]] <- objs %>%
  plotUMAP_split(., feature = "Phase", label.size=0, legend = T, reduction = 'umap')

umaps.rna[["Phase"]] <- ggarrange(plotlist = umaps.rna[["Phase"]], ncol=5, nrow=3, 
          common.legend = T, legend = "right") %>%
  annotate_figure(top = ggpubr::text_grob("Phase", face = "bold", size = 14))
```

```{r, fig.width=12, fig.height=8}
umaps.rna[["nFeature_RNA"]] <- objs %>%
  plotUMAP_split(., feature = "nFeature_RNA", label.size=0, legend = T, reduction = 'umap')

umaps.rna[["nFeature_RNA"]] <- ggarrange(plotlist = umaps.rna[["nFeature_RNA"]], ncol=5, nrow=3, 
          common.legend = T, legend = "right") %>%
  annotate_figure(top = ggpubr::text_grob("nFeature_RNA", face = "bold", size = 14))
```

```{r, fig.width=12, fig.height=8}
umaps.rna[["nCount_RNA"]] <- objs %>%
  plotUMAP_split(., feature = "nCount_RNA", label.size=0, legend = T, reduction = 'umap')

umaps.rna[["nCount_RNA"]] <- ggarrange(plotlist = umaps.rna[["nCount_RNA"]], ncol=5, nrow=3, 
          common.legend = T, legend = "right") %>%
  annotate_figure(top = ggpubr::text_grob("nCount_RNA", face = "bold", size = 14))
```

```{r, fig.width=12, fig.height=8}
umaps.rna[["log10GenesPerUMI"]] <- objs %>%
  plotUMAP_split(., feature = "log10GenesPerUMI", label.size=0, legend = T, reduction = 'umap')

umaps.rna[["log10GenesPerUMI"]] <- ggarrange(plotlist = umaps.rna[["log10GenesPerUMI"]], ncol=5, nrow=3, 
          common.legend = T, legend = "right") %>%
...
umaps.rna[["riboRatio"]] <- objs %>%
  plotUMAP_split(., feature = "riboRatio", label.size=0, legend = T, reduction = 'umap')

umaps.rna[["riboRatio"]] <- ggarrange(plotlist = umaps.rna[["riboRatio"]], ncol=5, nrow=3, 
          common.legend = T, legend = "right") %>%
...
umaps.rna[["mitoRatio"]] <- objs %>%
  plotUMAP_split(., feature = "mitoRatio", label.size=0, legend = T, reduction = 'umap')

umaps.rna[["mitoRatio"]] <- ggarrange(plotlist = umaps.rna[["mitoRatio"]], ncol=5, nrow=3, 
          common.legend = T, legend = "right") %>%
