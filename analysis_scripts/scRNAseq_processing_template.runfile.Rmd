...
```{r}
samplesheet <- read.table(here('analysis/bonemarrow/config/p23077_samplesheet.txt'), header=TRUE)
config <- yaml::yaml.load_file(here("analysis/bonemarrow/config/scRNA_config.yml"))
```
...
saveRDS(sc, '/Volumes/genomelab/illumina/runs/Analysis/2023_Analyses/p23077_Julia/p23077_Julia_Analysis/saved_rds/bonemarrow_soupchannels.Rds')
## always a good idea to reload the saved object to check that it saved ok
sc <- readRDS(here('analysis/bonemarrow/saved_rds/bonemarrow_soupchannels.Rds'))
```
...
```{r, include=FALSE}
## TODO: replace this with the gencoreSC findNormScaleVarFeatures
...
```{}
TODO: this is not very generalized yet, may need tweaking for your data
```


```{r}
qc_filtering_summary <- data.table::rbindlist(qc_receipts)[c(2,6),]
qc_filtering_summary <- as.data.frame(t(rbind(qc_filtering_summary, t(doubletfinder_results$Singlet), use.names = FALSE)))
colnames(qc_filtering_summary) <- c('initial', 'post_filtering', 'after_doublets')
qc_filtering_summary <- qc_filtering_summary %>% 
  mutate(low_qual_cells = initial - post_filtering) %>%
  mutate(doublets = post_filtering - after_doublets) %>%
  select(low_qual_cells, doublets, after_doublets)

```

```{r, fig.width=8}
reshape2::melt(as.matrix(qc_filtering_summary)) %>%
  ggplot(aes(x = Var1,
             y = value,
             group = Var2,
             fill = Var2)) + 
  ...
  scale_fill_manual(values = c('low_qual_cells' = 'grey',
                               'doublets' = 'goldenrod',
                               'after_doublets' = 'darkgreen'),
...
# Capture level clustering, UMAPS
