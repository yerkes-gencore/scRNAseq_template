...
<!-- If not hashed, you can comment the summary and detail chunks out -->
<!-- and use these simpler plots instead -->

<!-- # ```{r qc_metrics_table, eval=FALSE} -->
<!-- # samples <- read.table('samplesheet.txt', header=TRUE) -->
<!-- # qc_metrics <- capture_QC_metrics(samples) -->
<!-- # knitr::kable(qc_metrics) -->
<!-- # ``` -->
<!-- # -->
<!-- # ```{r qc_metrics_plot, fig.width=14, fig.height=4, eval=FALSE} -->
...
```{r}
if (!preran){
  saveRDS(combined.obj, file='saved_rds/combined_obj_postQC.Rds')
}
```
